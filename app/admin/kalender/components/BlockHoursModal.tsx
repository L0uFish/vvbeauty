...
export default function BlockHoursModal({
  ...
  const handleNext = (e: React.FormEvent) => {
    ...
    if (mode === "range") {
      if (!rangeFrom || !rangeTo) return alert("Selecteer een geldige periode.");
      parsed = generateDateRange(rangeFrom, rangeTo);
    } else {
      parsed = dates.filter(Boolean);
      if (parsed.length === 0) return alert("Voeg minstens één datum toe.");
    }
  ...
  const handleConfirm = async () => {
    const inserts = finalDates.map((d) => ({
      ...
      time_until: timeUntil + ":00",
      notes: notes || null,
      repeat_type: repeatType, // ✅ recurrence stored cleanly
    }));

    const { error } = await supabase.from("blocked_hours").insert(inserts);

    if (error) {
      console.error("❌ Fout bij opslaan geblokkeerde uren:", error);
      ...
      >
        {step === "form" && (
          <>
            ...
            <form onSubmit={handleNext} style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              ...
              {mode === "range" && (
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  <div style={{ display: "flex", gap: "10px" }}>
                    <div style={{ flex: 1 }}>
                      <label>Van</label>
                      <input type="date" value={rangeFrom} onChange={(e) => setRangeFrom(e.target.value)} required style={input} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <label>Tot</label>
                      <input type="date" value={rangeTo} onChange={(e) => setRangeTo(e.target.value)} required style={input} />
                    </div>
                  </div>
                  {selectedCount > 0 && (
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                <input type="time" value={timeFrom} onChange={(e) => setTimeFrom(e.target.value)} required style={{ ...input, flex: 1 }} />
                <input type="time" value={timeUntil} onChange={(e) => setTimeUntil(e.target.value)} required style={{ ...input, flex: 1 }} />
              </div>
              ...
              <select
                ...
                style={{ ...input, cursor: selectedCount > 1 ? "not-allowed" : "pointer", opacity: selectedCount > 1 ? 0.6 : 1 }}
              ...
              {selectedCount > 1 && (
                <p style={{ fontSize: "0.85rem", color: "#888", marginTop: "-4px" }}>
                  ⚠️ Herhaling enkel beschikbaar bij 1 geselecteerde dag.
                </p>
              )}

              <label style={{ fontWeight: 500 }}>Notitie (optioneel)</label>
              <input type="text" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="vb. Privéafspraak, verlof, ..." style={input} />

              <div style={btnRow}>
                <button type="button" onClick={onClose} style={{ ...button, background: "#fafafa", color: "#555", border: "1px solid #ddd" }}>
                  Annuleren
                </button>
                <button
                  type="submit"
        ...
        {step === "confirm" && (
          <div style={{ textAlign: "center" }}>
            <h3 style={{ color: "var(--vv-primary)", marginBottom: "14px" }}>Bevestig Geblokkeerde Uren</h3>
            <p><strong>Datums:</strong> {finalDates.map((d) => new Date(d).toLocaleDateString("nl-BE")).join(", ")}</p>
            <p><strong>Van:</strong> {timeFrom} – <strong>Tot:</strong> {timeUntil}</p>
